...
export function BulkUploadCategories() {
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = '/templates/categories-template.csv';
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      for (let i = 0; i < categories.length; i += batchSize) {
        ...
        const { error } = await supabaseAdmin.from('categories').insert(batch);
      ...
      alert(
        'Error uploading categories. Please check the CSV format and try again.'
      );
    } finally {
